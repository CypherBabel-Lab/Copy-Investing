...
contract ZeroExV2Adapter is AdapterBase, VaultFactoryOwnerMixin, ZeroExV2ActionsMixin {
    using SafeMath for uint256;
    event AllowedMakerAdded(address indexed account);

    event AllowedMakerRemoved(address indexed account);

    mapping(address => bool) private _makerToIsAllowed;
    ...
    constructor(address _integrationManager, address _exchange, address _vaultFactory, address[] memory _allowedMakers)
        AdapterBase(_integrationManager)
        VaultFactoryOwnerMixin(_vaultFactory)
    ...
    /// @param _vaultProxy The VaultProxy of the calling Vault
    ...
    function addAllowedMakers(address[] calldata _accountsToAdd) external onlyVaultFactoryOwner {
    ...
    function removeAllowedMakers(address[] calldata _accountsToRemove) external onlyVaultFactoryOwner {
        ...
        for (uint256 i; i < _accountsToRemove.length; i++) {
            ...
            _makerToIsAllowed[_accountsToRemove[i]] = false;
    ...
    function __addAllowedMakers(address[] memory _accountsToAdd) private {
        ...
        for (uint256 i; i < _accountsToAdd.length; i++) {
            ...
            _makerToIsAllowed[_accountsToAdd[i]] = true;
