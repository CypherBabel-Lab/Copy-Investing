...
abstract contract ChainlinkPriceFeedMixin {
    ...
    uint256 private constant _ETH_UNIT = 10 ** 18;

    uint256 private immutable _staleRateThreshold;
    address private immutable _wethToken;

    address private _ethUsdAggregator;
    mapping(address => AggregatorInfo) private _primitiveToAggregatorInfo;
    mapping(address => uint256) private _primitiveToUnit;

    constructor(address wethToken_, uint256 staleRateThreshold_) {
        _staleRateThreshold = staleRateThreshold_;
    ...
    function __setEthUsdAggregator(address _nextEthUsdAggregator) internal {
        ...
        _ethUsdAggregator = _nextEthUsdAggregator;
    ...
    function __calcConversionAmountEthRateAssetToUsdRateAsset(
        ...
        uint256 intermediateStep = _baseAssetAmount.mul(_baseAssetRate).mul(_ethPerUsdRate).div(_ETH_UNIT);
    ...
    function __calcConversionAmountUsdRateAssetToEthRateAsset(
        ...
        return intermediateStep.mul(_ETH_UNIT).div(_baseAssetUnit).div(_quoteAssetRate);
    }
    ...
    function __addPrimitives(
        ...
        for (uint256 i; i < _primitives.length; i++) {
            ...
            _primitiveToAggregatorInfo[_primitives[i]] =
            ...
            _primitiveToUnit[_primitives[i]] = unit;
    ...
    function __removePrimitives(address[] calldata _primitives) internal {
        for (uint256 i; i < _primitives.length; i++) {
            ...
            delete _primitiveToAggregatorInfo[_primitives[i]];
            delete _primitiveToUnit[_primitives[i]];
    ...
    function getAggregatorForPrimitive(address _primitive) public view returns (address aggregator_) {
        return _primitiveToAggregatorInfo[_primitive].aggregator;
    }
    ...
    function getRateAssetForPrimitive(address _primitive) public view returns (RateAsset rateAsset_) {
        ...
        return _primitiveToAggregatorInfo[_primitive].rateAsset;
    }

    /// @notice Gets the `_staleRateThreshold` variable value
    /// @return staleRateThreshold_ The `_staleRateThreshold` value
    function getStaleRateThreshold() public view returns (uint256 staleRateThreshold_) {
        return _staleRateThreshold;
    }
    ...
    function getUnitForPrimitive(address _primitive) public view returns (uint256 unit_) {
        ...
        return _primitiveToUnit[_primitive];
    }

    /// @notice Gets the `_wethToken` variable value
    /// @return wethToken_ The `_wethToken` variable value
